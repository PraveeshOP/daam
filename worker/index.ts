...
async function main() {
  ...
  priceWorker.on("completed", (job, result) => log("job", `${job.data.storeId} completed${result?.skipped ? " (skipped: already running)" : ""}`));
  priceWorker.on("failed", (job, error) => logError("job", `${job?.data.storeId ?? "unknown"} failed: ${error.message}`));
  ...
  notificationWorker.on("completed", (job) => log("notification-job", `alert ${job.data.alertId} completed`));
