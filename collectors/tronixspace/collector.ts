...
const STORE_API_URL = `https://tronixspace.com/wp-json/wc/store/v1/products?category=${TRONIXSPACE_GAMING_CATEGORY_ID}`;
...
export const tronixspaceCollector: StoreCollector = {
  ...
  async collect({ limit = 20 } = {}): Promise<CollectResult> {
    ...
    const items = await fetchAllWooCommerceItems<TronixspaceProduct>(STORE_API_URL, { maxItems: safeLimit });
